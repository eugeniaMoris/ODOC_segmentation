...
dst_data_path = '/data/'
dataset = 'RIGA'
sub_dataset = ['MagrabiFemale','MagrabiaMale']
...
def get_mask(file_name, final_name, img, subD):
    '''
    ...
    mask_paths = []
    for mask in sorted(glob.glob(proyect_path + or_data_path + dataset + '/Magrabia/' + subD + '/' + file_name + '-*' )):
    ...
    for idx in range(len(od_imgs)):
        ...
        iio.imwrite(proyect_path + dst_data_path + 'OD_extra/' + dataset + '-Magrabia' + '/' + str(idx+1) + '_' + final_name,od_imgs[idx])

        OC_add = np.add(OC_add,oc_imgs[idx])
        iio.imwrite(proyect_path + dst_data_path + 'OC_extra/' + dataset + '-Magrabia' + '/' + str(idx+1) + '_' + final_name,oc_imgs[idx])
    ...
    iio.imwrite(proyect_path + dst_data_path + 'OD1/' + dataset + '-Magrabia' + '/' + final_name,OD_add)
    iio.imwrite(proyect_path + dst_data_path + 'OC/' + dataset + '-Magrabia' + '/' + final_name,OC_add)
    

    return

def get_images(paths, names,subD):
    ...
    for p_i in range(len(paths)):
        ...
        #procesar
        iio.imwrite(proyect_path+dst_data_path+'images/' + dataset + '-Magrabia' + '/' +  names[p_i],img) #guardo

        get_mask(old_name, names[p_i], img, subD)
...
def main():
    ...
    for subD in sub_dataset:
        for img in sorted(glob.glob(proyect_path + or_data_path + dataset + '/Magrabia/' + subD + '/*prime.tif')):
        ...
        #print(img_paths)
        get_images(img_paths, final_img_name,subD)
...
# img_path = '/mnt/Almacenamiento/ODOC_segmentation/data/images/RIGA-MESSIDOR/001.png'
# OC_path = '/mnt/Almacenamiento/ODOC_segmentation/data/OC/RIGA-MESSIDOR/001.png'
# OD_path = '/mnt/Almacenamiento/ODOC_segmentation/data/OD1/RIGA-MESSIDOR/001.png'
